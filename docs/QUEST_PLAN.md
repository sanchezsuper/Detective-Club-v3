...
- **Годинник** — зупинився о **21:47** → перші чотири цифри PIN: `2147`
- **Піджак у крові** — у підкладці зашитий клаптик «…53» (крапки попереду показують, що це хвіст коду) → PIN: `214753`
- **Телефон** (HTML-сторінка за QR на «телефоні»-реквізиті) — знайдений, але **заблокований кодом з 4 цифр**. Поки що глухий кут: гравці бачать лише екран блокування. Це навмисно — телефон «вистрелить» після розсекречення
- **Зброя** — серійний номер: бонус-зачіпка, не частина PIN (див. «Бонус-зачіпки»)

> Чому телефон більше не дає цифр PIN: на екрані телефона забагато чисел (дата 12.04, час, дзвінок 20:47) — гравці плуталися б, які з них у PIN. Тепер джерела PIN лише два і вони чисті: годинник + клаптик.
...
```
Вводять `214753` → чорні блоки на очах зникають → службова записка № 117-С:
- Майор **Роман Гайдук, 19 серпня 1978 року народження**, група крові II (A) Rh+, псевдонім «Сокіл»
- Довідка архіваріуса: *за стандартом Служби Х код блокування службових телефонів — день і місяць народження власника*
- У сховищі телефону — робочі нотатки «Периметр»

### Крок 8-Б · Телефон жертви
Гравці повертаються до заблокованого телефона: `19.08` → код **1908** → телефон відкривається:
...
- нотатки **«Периметр»: три точки з радіусами** — їх переносять на паперову мапу (точки підставите під свою місцевість)
...
|---|---|---|
| 1 | **Підказка про стандарт коду телефона.** Зараз у 117-С прямо написано «код = день і місяць народження власника». Лишити (надійно) чи прибрати, щоб гравці самі здогадалися по даті народження (складніше, але є ризик застрягання)? | Зараз підказка Є. Мій голос — лишити: диспетчер завжди може забрати підказку, а от додати її в зашифрований блок на льоту не можна |
| 2 | **Порядок цифр PIN.** Годинник дає 2147, клаптик «…53» — крапки попереду мають показати, що це хвіст. Достатньо очевидно? | Пропоную на плейтесті перевірити; запасна підказка диспетчера: «почни з часу» |
| 3 | **Зброя.** Моя пропозиція — бонус-зачіпка Б-3 (картка списання). Ви ще думали про «цифри на ній» | Вирішити: бонус (моя рекомендація) чи резервний канал цифр PIN на випадок зірваної гілки |
| 4 | **Ролі** (Криптограф/Картограф/…) | Досі відкрито: косметичний бейдж у кабінеті чи прибрати зовсім. Механіку під ролі не будуємо в будь-якому разі |
| 5 | **Дзвінок о 20:47** | Пропоную зробити частиною бонус-зачіпки Б-1 «час не б'ється», а не просто флейвором. Потребує вашого ОК |
| 6 | **Тривалість.** 3 години закладено в таймер | Фіналізувати після соло-прогону: заміряти кожну гілку |
...
| 8 | **Точки тріангуляції** у нотатках телефона — зараз плейсхолдери `[ЗАМІНИТИ]` | Чекають вашої локації; після заміни онлайн-частина готова до бою |
...
ВУЛИЦЯ, РІВЕНЬ 1 (докази → PIN розсекречення):
  Годинник   зупинився о 21:47      → [2][1][4][7]
  Піджак     клаптик «…53»          → [5][3]
                                      ──────────
  PIN: 214753 → чорні блоки зникають → записка 117-С

РОЗСЕКРЕЧЕНЕ ДОСЬЄ → КОД ТЕЛЕФОНА:
  «19 серпня 1978 р.н.» + стандарт Служби (день+місяць)
  → код телефона: 1908 → нотатки «Периметр»: 3 точки з радіусами

ВУЛИЦЯ, РІВЕНЬ 2:
  3 точки + паперова мапа + циркуль → точка Х → розкопки → контейнер
...
| Б-3 | **Зброя зі складу Служби** | Серійний № на зброї (реквізит) = № у «картці списання» (паперовий реквізит у конверті «АРХІВ», датований 3 роками раніше) | Зброю міг узяти лише свій — жодного «зовнішнього вбивці» |
...
flowchart TD
    ...
    B["ФАЗА B · Веб-частина ✅ (прототип готовий)<br/>— лендінг · реєстрація · дитячий допуск<br/>— кабінет: картка, лобі, СТАРТ, таймер<br/>— непримітна кнопка + термінал (кіно-UI)<br/>— кнопка «i»: завдання логін/пароль<br/>— досьє з реальним розсекреченням по PIN<br/>— телефон із кодовим замком 1908<br/>— «ДОСТУП ЗАБОРОНЕНО» (red herring)<br/>— «СПРАВУ РОЗКРИТО» + бігучий рядок (твіст 2)<br/>— лишилось: точки «Периметра» під локацію"]
    C["ФАЗА C · Реквізит<br/>— паперове досьє + маркер + UV-написи<br/>— годинник (зупинити на 21:47)<br/>— піджак: клаптик «…53» + бирка «експрес-тест I (0) Rh−» (Б-2)<br/>— «телефон»-реквізит з QR на сторінку<br/>— зброя із серійником + картка списання (Б-3)<br/>— колесо шифру, паперова мапа<br/>— посвідчення «Мирон Гай» + лист<br/>— поліцейська стрічка, конверти"]
...
flowchart TD
    ...
    subgraph S2["ЕТАП 2 · Місце злочину (вулиця)"]
        D0 --> D1["Годинник: зупинився о 21:47 → 2147"]
        D0 --> D2["Піджак: клаптик «…53» → 53"]
        D0 --> D3["Телефон: ЗАБЛОКОВАНИЙ<br/>код 4 цифри — поки глухий кут"]
        D0 --> D4["Зброя: серійний № → бонус Б-3"]
    end
    D1 --> E0["PIN 214753 — вводять на сторінці досьє"]
    D2 --> E0
    E0 --> E1["РОЗСЕКРЕЧЕННЯ · записка 117-С:<br/>Гайдук, 19.08.1978, група крові II (A),<br/>стандарт Служби: код = день+місяць народження"]
    E1 --> E2["Код телефона: 1908"]
    D3 --> E2
    E2 --> E2b["Нотатки «Периметр»:<br/>3 точки з радіусами + дзвінок 20:47 (бонус Б-1)"]
    E2b --> E2c["Точки переносять на паперову мапу,<br/>циркуль → точка X"]
